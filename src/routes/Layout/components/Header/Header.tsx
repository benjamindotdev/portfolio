            ...
            <ul className="flex flex-row items-center gap-8 p-0 list-none">
                {links.map((link) => (
                    <li key={link.title} className="group">
                        <NavLink
                            className="text-slate-700 dark:text-portfolio-white no-underline text-portfolio-h3 font-lunasima group-hover:!text-portfolio-green transition-all duration-500 hover:scale-110"
                            to={link.route}
                            style={({ isActive }) => {
                                return {
